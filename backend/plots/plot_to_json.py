...
class PlotGenerator:
    ...
    def generate_plot(self, csv_path: str, json_folder: str) -> str:
        ...
        try:
            ...
            # Save the plot as JSON and get the JSON string
    ...
    def generate_data_by_name(self, data_name: str) -> str:
        ...
        if data_name.lower() == 'compression ratio':
            ...
            csv_file = os.path.join(base_dir, 'data', 'Result_data', 'C_CPU.csv')
            json_dir = os.path.join(base_dir, 'data', 'plot_metadata')
            try:
                return self.generate_plot(csv_file, json_dir)
            except Exception as e:
                print(f"An error occurred in generate_data_by_name: {e}")
                raise
        else:
